...
| 2026-08-10 | Android 10 IME lifecycle crash [#2555](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2555) | No upstream relationship inferred | Detached-view checks and `RuntimeException` fallbacks now cover delayed focus, `restartInput`, `isActive`, and soft-input operations. Android source tests and compilation pass. A fresh API 35 AVD diagnostic passes for virtual-display and hybrid composition, reopening a Flutter keyboard with `keyboardInsetAfterDispose=24.0` after WebView clear/dispose; no AndroidRuntime/fatal/IME NPE appears. Android 10 physical-device and OEM validation remains pending. |
