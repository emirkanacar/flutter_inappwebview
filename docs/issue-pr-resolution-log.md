...
| 2026-08-10 | iOS/Android disposal crash [#2654](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2654) | No upstream relationship inferred | iOS `InAppWebView.dispose()` and Android native WebView disposal are now idempotent; Android fullscreen cleanup still runs before destroy. Source tests and native builds pass. Clean iPhone 17 Pro iOS 27 Simulator and fresh API 35 Android `flutter drive` runs complete four disposal/recreate cycles; Android emits renderer exit code `-1` during explicit teardown but exits with no app `AndroidRuntime`, fatal, ANR, or Dart failure. This matches the external #2491 renderer signature, but its exact back-button/OEM path remains pending; physical iOS/Android teardown and renderer validation remain required. |
