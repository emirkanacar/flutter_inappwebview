...
The export contains 125 issues and 73 PRs. Fifty-eight issue records are now resolved or mitigated locally and are documented in the [resolution log](issue-pr-resolution-log.md). Three additional PR-only records (`#2771`, `#2871`, and `#2474`) are implemented locally; they are tracked separately and do not change the issue counts below. The remaining issue records are:

| Category | Count | Treatment |
| --- | ---: | --- |
| Bugs | 55 | Technical work, validation, or reproduction required |
...
| **Total remaining issue records** | **67** | **64 technical issue records after excluding showcase entries** |
