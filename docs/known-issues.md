...
| Open or needs reproduction | [#2831](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2831) | The report has no plugin-owned iOS geolocation callback or presenter path in source; affected-device reproduction is still required before changing unrelated permission code. |
...
**Local status:** Open only for affected-device reproduction. **Affected scope:** iOS/WebKit system geolocation prompt. **Impact:** the upstream report says the native location dialog appears but its buttons cannot be tapped. The reported `onGeolocationPermissionsShowPrompt` callback is Android-only in the platform contract, and the iOS native package contains no geolocation permission delegate or plugin-owned presenter for this dialog. Existing scene-aware prompt paths guard missing visible view controllers, but they do not own the system location alert. **Required evidence:** reproduce the minimal geolocation page on iOS 26 with active/inactive scenes, popup presentation, dismissal, and background/foreground transitions before changing shared permission code.
...
| Device/native validation | [#2861](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2861), [#2763](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2763) | The local fix exists; run Fedora/X11/Intel Linux and iOS popup matrices before declaring runtime behavior fully validated. |
| Reproduction required | [#2831](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2831) | Do not change shared iOS permission code without a platform-specific reproduction and diagnostic evidence. |
| Source review complete | [#2745](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2745), [#2536](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2536) | Dynamic JavaScript evaluation is an explicit API boundary; Android activity extras use only Bundle primitives and nested Bundles. Reopen only with a concrete source-to-sink trace. |
...
**Status:** #2710 is mitigated in iOS 2.1.0; #2831 remains open for device reproduction and #2763 is fixed in source pending popup-device validation. **Impact:** User-visible iOS regressions: fullscreen video can turn black/unresponsive, location prompts may not close, and rejected `onCreateWindow` targets could navigate the caller. **Confidence:** Strong symptoms; the remaining location prompt behavior is likely iOS/WebKit/system-owned.
...
[#2831](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2831) still needs an OS/SDK-specific reproduction before changing shared permission code. [#2763](https://github.com/pichillilorenzo/flutter_inappwebview/issues/2763) now has a source fix and needs only the iOS popup device matrix. Maintain an iOS matrix for Flutter, Xcode, iOS, keyboard, fullscreen, and multi-window flows; do not present these as one common root cause.
