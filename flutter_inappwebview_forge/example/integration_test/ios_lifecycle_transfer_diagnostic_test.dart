...
const _runDiagnostic = bool.fromEnvironment(
  'RUN_IOS_LIFECYCLE_TRANSFER_DIAGNOSTIC',
);
const _diagnosticCycles = int.fromEnvironment(
  'IOS_LIFECYCLE_TRANSFER_CYCLES',
  defaultValue: 50,
);
...
<html>
<body>
<p>iOS lifecycle transfer diagnostic</p>
<script>
...
void main() {
  ...
  testWidgets(
    'iOS keepAlive and headless ownership survive repeated transfer',
    (WidgetTester tester) async {
      final keepAlive = InAppWebViewKeepAlive();
      InAppWebViewController? retainedController;

      for (var cycle = 0; cycle < _diagnosticCycles; cycle++) {
        debugPrint('iOS lifecycle transfer: keepAlive cycle=$cycle');
        final controllerCompleter = Completer<InAppWebViewController>();
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
              body: InAppWebView(
                key: ValueKey<String>('ios-retained-$cycle'),
                keepAlive: keepAlive,
                initialData: cycle == 0
      ...
      for (var cycle = 0; cycle < _diagnosticCycles; cycle++) {
        debugPrint('iOS lifecycle transfer: headless cycle=$cycle');
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
              body: InAppWebView(
                key: ValueKey<String>('ios-headless-$cycle'),
