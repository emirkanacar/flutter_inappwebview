...
public class Util {
  ...
  private static final long SYNC_METHOD_CHANNEL_TIMEOUT_MILLIS = 500L;
  ...
  public static <T> T invokeMethodAndWaitResult(final @NonNull MethodChannel channel,
    ...
    if (Looper.myLooper() == Looper.getMainLooper()) {
      // A synchronous wait on the main looper prevents the result callback from
      // ever running and can freeze the WebView/UI thread. Let the callback
      // complete asynchronously and use the native default behaviour instead.
    ...
    if (!callback.latch.await(SYNC_METHOD_CHANNEL_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
